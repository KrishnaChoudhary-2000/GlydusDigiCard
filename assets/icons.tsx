...
export const GlydusLogo: React.FC<{ className?: string, isCardLogo?: boolean }> = ({ className, isCardLogo }) => {
    // When used as a fallback for an image on the card, we render nothing,
    // as per the request to remove the image.
    if (isCardLogo) {
        return null;
    }

    // For other UI elements like the header, render the text version of the logo.
    return (
        <div className={`flex flex-col font-extrabold tracking-tighter ${className}`}>
            <span className="text-3xl leading-none text-white">GLYDUS</span>
            <span className="text-[10px] font-medium leading-none text-brand-accent tracking-widest mt-1.5">
                MAKE WAVES
            </span>
        </div>
    );
